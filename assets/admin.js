...
$('#email_send_btn')?.addEventListener('click', async () => {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: {
        'Authorization': 'Bearer re_YqS6m9mW_7HBJZqHWxT23iAWZh5RPyKAR',
      ...
      body: JSON.stringify({
        ...
        html: `<!DOCTYPE html><html><body style="margin:0;padding:40px 16px;background:#060912;font-family:ui-sans-serif,system-ui,Arial;color:#e6e8ee;">
          <div style="max-width:560px;margin:0 auto;background:#0b1020;border:1px solid rgba(255,255,255,.08);border-radius:18px;padding:40px 36px;">
            ...
            <div style="color:#e6e8ee;font-size:.95rem;line-height:1.7;white-space:pre-wrap;">${body}</div>
    ...
    if (!res.ok) throw new Error(data.message || data.name || 'Send failed');
